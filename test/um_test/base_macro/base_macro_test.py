...
class BaseMacroTest(unittest.TestCase):
    def test_start(self):
        base_macro = BaseMacro(MockInputCollector(), 10, False)
    ...
    def test_timeout(self):
        base_macro = BaseMacro(MockInputCollector(), 0.1, False)
        t = Thread(target=base_macro.start)
        t.start()
        sleep(0.15)
        self.assertFalse(t.is_alive())
    ...
    def test_shortcut_termination(self):
        ...
        base_macro = BaseMacro(MacroEventCollector(mock_input_collector), 10, False)
